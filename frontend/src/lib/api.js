/**
 * API 调用封装（ESM 版，原 window.APP_API）
 * 字段名适配 snake_case → camelCase；函数体逐字不变
 */
...
export const loadChats = async (backendUrl) => {
    try {
...
export const loadArchivedChats = async (backendUrl) => {
    try {
...
export const activateChat = async (backendUrl, sessionId) => {
    try {
...
export const getChatDetail = async (backendUrl, chatId) => {
    try {
...
export const deleteChat = async (backendUrl, chatId) => {
    try {
        const response = await fetch(`${backendUrl}/session/${chatId}`, {
...
export const uploadFile = async (backendUrl, file) => {
...
export const getStreamChatUrl = (backendUrl, selectedAgent, hasFile) => {
...
export const stopStream = async (backendUrl, conversationId) => {
    try {
